...
describe("<CoordinateInput />", () => {
  ...
  describe("longitude input", () => {
    const testLongitudeDataValues: CoordinateInputProps = {
      dataTestId: "longitudeInput",
      label: "test label",
      value: 20.12345,
    ...
    const { dataTestId, label, value } = testLongitudeDataValues;

    describe("initial render", () => {
      beforeEach(() => {
        renderWithTheme(
          <CoordinateInput
            {...testLongitudeDataValues}
            setNumberState={setMockState}
          />,
          theme
        );
      ...
      test("should have attribute: min=-180", () => {
        const inputField = screen.getByTestId(dataTestId);
        expect(inputField).toHaveAttribute("min", "-180");
      });
      test("should have attribute: max=180", () => {
        const inputField = screen.getByTestId(dataTestId);
        expect(inputField).toHaveAttribute("max", "180");
      });
